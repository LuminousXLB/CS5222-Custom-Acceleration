...
def parse_args():
    parser = argparse.ArgumentParser(
        description="produce synthesis constraints from mnist training data"
    )
    parser.add_argument(
        "--data-dir", type=str, default="mnist/", help="the input data directory"
    )
    parser.add_argument(
        "--num-examples", type=int, default=8, help="the number of training examples"
    )
    parser.add_argument(
        "--dim",
        type=int,
...
if __name__ == "__main__":
    ...
    # Produce stats
    print(
        "Min/Max of coefficient values [{}, {}]".format(
            reg.coef_.min(), reg.coef_.max()
        )
    )
    print(
        "Min/Max of intersect values [{}, {}]".format(
            reg.intercept_.min(), reg.intercept_.max()
        )
    )
    print(
        "Misclassifications (float) = {0:.2f}%".format(
            float_errors / len(test_labels) * 100
        )
    )
    print(
        "Misclassifications (fixed) = {0:.2f}%".format(
            fixed_errors / len(test_labels) * 100
        )
    )

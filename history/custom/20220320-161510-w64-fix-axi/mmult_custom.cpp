...
#else
    out_T offset[CLASSES + (CLASSES % OUT_WIDTH_RATIO)];
    out_T out_buf[TILING][CLASSES + (CLASSES % OUT_WIDTH_RATIO)];
#endif
...
#define LAST (((t + i) == (BATCH - 1)) && (j == CLASSES * (CLASSES / OUT_WIDTH_RATIO)))
                if (LAST) {
                    printf("FPGA PUSHING %d (last=%d)\n", t + i, LAST);
                }
                push_stream(out_stream, packet, LAST);
